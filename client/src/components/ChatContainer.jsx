    ...
    return selectedUser ? (
        <div className='h-full overflow-scroll relative backdrop-blur-lg'>
            <div className="flex items-center gap-3 py-3 mx-4 border-b border-stone-500">
                ...
                <img src={assets.help_icon} alt="" className='md:hidden w-5' />
            </div>
            <div className="flex flex-col h-[calc(100%-120px)] overflow-scroll p-3 pb-6">
                {messages.map((msg, index) => (
                      msg && (
                    <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId.toString() !== authUser._id.toString()
                     ? 'flex-row-reverse' : ''}`}> 
